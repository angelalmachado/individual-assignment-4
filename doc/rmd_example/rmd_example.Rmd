...
Additionally, note that in \@ref(tab:horses_sd_table) we consider the sample standard deviation of the number of horses during the same time span as \@ref(fig:horse_pops_plot).

```{r horse_plot_largest, echo=FALSE, out.width="50%", fig.cap="Horse populations for the province with the largest standard deviation"}
knitr::include_graphics("../../results/horse_pop_plot_largest_sd.png")
```

In \@ref(fig:horse_plot_largest) we zoom in and look at the province of ?, which had the largest spread of values in terms of standard deviation.
